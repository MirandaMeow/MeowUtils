...
public class CommandSeal implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String s, String[] args) {
        if (args.length != 2) {
            sender.sendMessage("§e用法 §6/sealscroll §b[player] <amount>");
            return true;
        }
        ...
        if (miss == 0) {
            ...
            sender.sendMessage(String.format("§e将 §b%s §e个§9§l封印卷轴§r§e给 §b%s §e, 其中 §b%d §e个因为空间不足已丢失", amount, playerName, miss));
            player.sendMessage(String.format("§e收到了 §b%s §e个§9§l封印卷轴§r§e, 其中 §b%d §e个因为空间不足已丢失", amount, miss));
        }
        return true;
    }
